...
* Improved Pro List View thumbnails for FolioBlocks Image Blocks, Video Blocks, and gallery children.
* Added a unified Image Click Behavior control for Image Block and photo galleries, with support for Lightbox, Downloads, WooCommerce products, Media File links, Custom URLs, and Page/Post links.
* Added faster per-image linking workflows, including toolbar shortcuts for Custom URL, Page/Post, and WooCommerce product links.
* Added EXIF metadata support for Image Blocks, with premium Lightbox and hover overlay display options.
* Improved Image Hover Settings with a single Hover Style control, new overlay content options, and refined color controls.
* Updated Filmstrip, Carousel, Grid, Masonry, and Justified galleries to use the improved click and hover settings patterns.
* Improved gallery filtering controls, including clearer per-image filtering settings.
* Refreshed the FolioBlocks admin dashboard and fixed the admin menu icon display.
* Fixed Masonry Gallery column settings on the front end, including desktop, tablet, and mobile column values.
* Fixed front-end rendering when multiple Masonry or Justified galleries appear on the same page.
* Fixed Justified Gallery filtering so filter bars only affect their own gallery.
* Fixed hover overlay display issues, including Gradient Overlay behavior when Reduce Motion is enabled.
* Fixed several Image Block inspector, Lightbox, EXIF, WooCommerce, and overlay style issues.
...
= 1.2.4 =
* Excludes System Info from translation strings
