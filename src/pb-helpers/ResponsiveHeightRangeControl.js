...
const ResponsiveHeightRangeControl = ({
	...
	const getValue = () => {
		if (device === 'tablet') return safeTablet;
		if (device === 'mobile') return safeMobile;
		return safeDesktop;
	};
	...
	const getMax = () => {
		if (device === 'tablet') return resolvedMaxTablet;
		if (device === 'mobile') return resolvedMaxMobile;
		return maxDesktop;
	};
