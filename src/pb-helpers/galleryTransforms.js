...
const getTransformAttributes = (attributes = {}) => {
	...
	Object.entries(attributes).forEach(([key, value]) => {
		if (EXCLUDED_ATTRS.has(key)) return;
...
export const disableGalleryWrapperTransforms = () => {
	if (hasPatchedWrapperTransforms) return;
	patchAttempts += 1;
	let allFound = true;
	...
	DISABLED_TARGET_BLOCKS.forEach((targetName) => {
		...
		const patchedFrom = blockType.transforms.from.map((transform) => {
			if (transform.type !== 'block') return transform;
			if (transform[TRANSFORM_PATCH_FLAG]) return transform;
			...
			const patched = {
				...
				isMatch: (attributes, block) => {
					...
					if (hasFolioBlocks) return false;
					return originalIsMatch ? originalIsMatch(attributes, block) : true;
				},
			};
