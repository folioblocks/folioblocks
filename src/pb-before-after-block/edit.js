...
export default function Edit({ attributes, setAttributes }) {
	...
	useEffect(() => {
		if (beforeImage?.sizes) {
			const newSrc =
				beforeImage.sizes[attributes.resolution]?.url || beforeImage.src;
			setAttributes({
				beforeImage: {
					...beforeImage,
					src: newSrc,
		...
		if (afterImage?.sizes) {
			const newSrc =
				afterImage.sizes[attributes.resolution]?.url || afterImage.src;
			setAttributes({
				afterImage: {
					...afterImage,
					src: newSrc,
		...
		<>
			...
			{beforeImage?.src && afterImage?.src && (
				<>
					<InspectorControls>
						<PanelBody
							title={__("Before & After Block Settings", "folioblocks")}
							initialOpen={true}
						>
							...
							<SelectControl
								...
								options={[
									{
										label: __("Horizontal", "folioblocks"),
										value: "horizontal",
									},
									{
				...
				{beforeImage?.src && afterImage?.src ? (
					<div
						className={`pb-before-after-container ${
							sliderOrientation === "vertical" ? "is-vertical" : ""
						}`}
						ref={containerRef}
					>
						<div className="pb-after-wrapper">
							<img
								...
								alt={afterImage.alt}
							/>
							{applyFilters("folioBlocks.beforeAfter.renderAfterLabel", null, {
								attributes,
							})}
						</div>
						<div
						...
						>
							<img
								...
								alt={beforeImage.alt}
							/>
							{applyFilters("folioBlocks.beforeAfter.renderBeforeLabel", null, {
								attributes,
							})}
						</div>
						<input
