...
addFilter(
	...
	( defaultContent, props ) => {
		...
		return (
			<RangeControl
				label={ __(
					'Drag Handle Starting Position (%)',
					'folioblocks'
				) }
				value={ attributes.startingPosition }
				onChange={ ( value ) =>
					setAttributes( { startingPosition: value } )
				}
				min={ 0 }
				max={ 100 }
				...
				help={ __(
					'Set starting position for slider drag handle.',
					'folioblocks'
				) }
			/>
		);
			...
			<>
				...
				{ showLabels && (
					<SelectControl
						...
						help={ __(
							'Set Before & After label position.',
							'folioblocks'
						) }
						__nextHasNoMarginBottom
						__next40pxDefaultSize
...
addFilter(
	...
	( defaultContent, props ) => {
		const { setAttributes, attributes } = props;
		const { sliderColor, labelTextColor, labelBackgroundColor } =
			attributes;
		...
		return (
			<PanelColorSettings
				...
				colorSettings={ [
					{
						label: __(
							'Slider Handle & Line Color',
							'folioblocks'
						),
						value: sliderColor,
						onChange: ( value ) =>
							setAttributes( { sliderColor: value } ),
					},
					{
					...
					},
					{
						...
						onChange: ( value ) =>
							setAttributes( { labelBackgroundColor: value } ),
					},
				] }
			/>
		);
