...
const getImageSrcForResolution = (block, resolution) =>
	block?.attributes?.sizes?.[resolution]?.url || block?.attributes?.src || "";
...
export default function Edit({ attributes, setAttributes, clientId }) {
	...
	useEffect(() => {
		...
		const timer = setInterval(() => {
			setActiveIndex((current) => (current + 1) % innerBlocks.length);
		}, intervalMs);
	...
	const onSelectImages = async (media) => {
		...
		const newBlocks = imagesToAdd.map((image) => {
			...
			return wp.blocks.createBlock("folioblocks/pb-image-block", {
				...
				title:
					titleMap[image.id] ||
					decodeEntities(image.title?.rendered || image.title || ""),
				caption,
				width,
				height,
				sizes: image.sizes || {},
	...
	const activeImageTitle = String(activeBlock?.attributes?.title || "").trim();
	const activeImageProductId = Number(activeImageAttributes.wooProductId || 0);
	...
	if (isLoading) {
		...
		editorContent = (
			<div className={`pb-filmstrip-gallery-preview is-${filmstripPosition}`}>
				<div
				...
				>
					{hasMultipleImages && (
						<>
							<button
								...
								onClick={() => setActiveImage(activeIndex - 1, true)}
								aria-label={__("Previous image", "folioblocks")}
							>
								&#8249;
							</button>
							<button
						...
						<div className="pb-filmstrip-gallery-bottom-controls">
							{autoplay && (
								<button
									...
									aria-label={
										isPlaying
											? __("Pause autoplay", "folioblocks")
											: __("Play autoplay", "folioblocks")
									}
								>
									{isPlaying ? (
										<svg
							...
							{enableFullscreen && (
								<button
									...
									aria-label={__("Open Fullscreen", "folioblocks")}
								>
					...
					{applyFilters("folioBlocks.imageBlock.downloadButton", null, {
						...
						effectiveDownloadOnHover: attributes.downloadOnHover ?? true,
						sizes: activeImageSizes,
						src: activeImageSrc,
						context: {
							"folioBlocks/enableDownload": attributes.enableDownload,
							"folioBlocks/downloadOnHover": attributes.downloadOnHover ?? true,
						},
						isInsideGallery: true,
						downloadIconStyleVars,
						effectiveDownloadIconColor: attributes.downloadIconColor ?? "",
						effectiveDownloadIconBgColor: attributes.downloadIconBgColor ?? "",
					})}
					{applyFilters("folioBlocks.imageBlock.addToCartButton", null, {
						...
						context: {
							"folioBlocks/wooCartIconDisplay": attributes.wooCartIconDisplay,
							"folioBlocks/wooDefaultLinkAction":
								attributes.wooDefaultLinkAction,
						},
						isInsideGallery: true,
						cartIconStyleVars,
						effectiveCartIconColor: attributes.cartIconColor ?? "",
						effectiveCartIconBgColor: attributes.cartIconBgColor ?? "",
					})}
					...
					{activeImageSrc ? (
						...
						<div className="pb-filmstrip-gallery-main-empty">
							{__("Select an image to preview it.", "folioblocks")}
						</div>
					)}
					{showOverlay && (
				...
				<div className="pb-filmstrip-gallery-thumbnails-wrapper">
					{showStripArrowsBottom && (
						<button
							...
							onClick={() => setActiveImage(activeIndex - 1, true)}
							aria-label={__("Previous thumbnail", "folioblocks")}
						>
							&#8249;
						</button>
					)}
					...
					<div
						...
						role="tablist"
						aria-label={__("Filmstrip thumbnails", "folioblocks")}
					>
						{innerBlocks.map((block, index) => {
							const thumbnailSrc = getImageSrcForResolution(block, "thumbnail");
							const fallbackSrc = getImageSrcForResolution(block, resolution);
							...
							return (
								<button
									...
									onClick={() => setActiveImage(index, true)}
								>
									{src ? (
										<img
											src={src}
											alt={
												...
												__("Gallery thumbnail", "folioblocks")
											}
										/>
									) : (
										<span>{index + 1}</span>
			...
			<InspectorControls>
				...
				>
						...
						onChange={(newResolution) => {
							...
							innerBlocks.forEach((block) => {
								updateBlockAttributes(block.clientId, {
									src: getImageSrcForResolution(block, newResolution),
					...
					<SelectControl
						...
						onChange={(value) => setAttributes({ filmstripPosition: value })}
						help={__(
					...
					{applyFilters(
						...
						<div style={{ marginBottom: "8px" }}>
							<Notice status="info" isDismissible={false}>
								<strong>{__("Enable Full-Screen Mode", "folioblocks")}</strong>
								<br />
								{__(
									"This is a premium feature. Unlock all features:",
									"folioblocks",
			...
			<InspectorControls group="styles">
				...
				>
					{applyFilters(
						...
						<div style={{ marginBottom: "8px" }}>
							<Notice status="info" isDismissible={false}>
								<strong>{__("Color Mode", "folioblocks")}</strong>
								<br />
								{__(
									"This is a premium feature. Unlock all features:",
									"folioblocks",
				...
				{applyFilters("folioBlocks.filmstripGallery.iconStyleControls", null, {
					attributes,
					setAttributes,
				})}
			</InspectorControls>
