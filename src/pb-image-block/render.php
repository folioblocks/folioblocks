...
if ( pb_fs()->can_use_premium_code__premium_only() ) {
	...
	$woo_cart_display = $pb_context['folioBlocks/wooCartIconDisplay'] ?? 'always';
	$woo_hover_info = $pb_context['folioBlocks/wooProductPriceOnHover'] ?? false;
	$woo_lightbox_info = $pb_context['folioBlocks/wooLightboxInfoType'] ?? 'caption';

	$lazy_load = $block->context['folioBlocks/lazyLoad'] ?? false;
	$pb_loading_attr = $lazy_load ? 'lazy' : 'eager';
	$dropshadow       = $pb_context['folioBlocks/dropShadow'] ?? ! empty( $attributes['dropshadow'] );


	$filter_category = $attributes['filterCategory'] ?? '';

	$border_width  = isset( $pb_context['folioBlocks/borderWidth'] ) ? $pb_context['folioBlocks/borderWidth'] : ( $attributes['borderWidth'] ?? 0 );
	$border_radius = isset( $pb_context['folioBlocks/borderRadius'] ) ? $pb_context['folioBlocks/borderRadius'] : ( $attributes['borderRadius'] ?? 0 );
	$border_color  = isset( $pb_context['folioBlocks/borderColor'] ) ? $pb_context['folioBlocks/borderColor'] : ( $attributes['borderColor'] ?? '#ffffff' );
	...
	>
		<?php if ( $pb_lightbox ) : ?>
			...
			if ( $pb_caption_lightbox ) {
				...
				if ( ! isset( $pb_lightbox_caption ) && ! empty( $pb_caption ) ) {
        			$pb_lightbox_caption = wpautop( wp_kses_post( $pb_caption ) );
    			}
			}
			?>
			...
			>
				...
				if ( pb_fs()->can_use_premium_code__premium_only() ) {
    				$pb_img_attributes['style'] = $img_styles;
				}
				if ( ! empty( $pb_alt ) ) {
		...
		<?php
			...
			if ( pb_fs()->can_use_premium_code__premium_only() ) {
				...
				if ( ! pb_fs()->can_use_premium_code__premium_only() && ! empty( $pb_title ) ) {
					echo '<div class="pb-image-block-title-container">';
		...
		<?php endif; ?>
		<?php if ( pb_fs()->can_use_premium_code__premium_only() ) : ?>
			<?php if ( ( ! $enable_woo ) && ( ! empty( $pb_context['folioBlocks/enableDownload'] ) || ! empty( $attributes['enableDownload'] ) ) ) : ?>
	    		<button 
