		...
		const calculateLayout = () => {
			...
			rows.forEach((row) => {
				...
				const scale = shouldScale
					? Math.min((containerWidth - totalGaps) / totalScaledWidth, 1)
					: 1;
				...
				row.forEach((img, index) => {
					...
					const inlineMarginValue =
						!isLastInRow && !attributes.noGap ? `${gap}px` : "0px";
					const blockMarginValue = !attributes.noGap ? `${gap}px` : "0px";
					img.wrapper.style.setProperty(
						"--pb-margin-inline",
						inlineMarginValue,
					);
					img.wrapper.style.setProperty("--pb-margin-block", blockMarginValue);
				});
			});
		};
	...
	return (
		<>
			...
			<InspectorControls group="styles">
				...
				{applyFilters("folioBlocks.justifiedGallery.iconStyleControls", null, {
					attributes,
					setAttributes,
				})}
			</InspectorControls>
